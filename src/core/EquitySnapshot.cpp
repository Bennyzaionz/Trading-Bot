...
double EquitySnapshot::getPrice(const PriceType price_type) const
{
    if( price_type == PriceType::LAST )
        ...
        return getLow();
    else if( price_type == PriceType::HIGH )
        return getHigh();
    else if( price_type == PriceType::BID )
        return getBid();
    else if( price_type == PriceType::ASK )
        return getAsk();
...
void EquitySnapshot::print(const PrintType print_type) const
{

    // std::cout << "---------- EquitySnapshot ----------" << std::endl;

    if ( print_type == PrintType::TRADE )
    {
